...
def to_tile_square(img, size, 
    ...
    # return:
    # img:   PIL(3, size, size)
    # location: arr(6) : ymin, xmin, ymax, xmax, ori_h, pori_w
    # boxes: Tensor(n, 4) or None : ymin, xmin, ymax, xmax
    ...
    if mode == 'TRAIN':
        max_ofst_h = size - oh
        max_ofst_w = size - ow
        ofst_h = random.randint(0, max(max_ofst_h-2, 0))
        ofst_w = random.randint(0, max(max_ofst_w-2, 0))
...
def to_stretch_square(img, size, move_size, 
                boxes=None, masks=None, mode='TRAIN'):
    # size:  int
    # transfer_p: float (0~1)
    # transfer_min: float (0~1)
