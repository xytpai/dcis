...
class ResNet(nn.Module):
    def __init__(self, depth, num_classes=1000, use_dcn=False):
        dcn_layers = [0, 0, 0, 0]
        dcn_interval = 1
        if depth == 50: 
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], 
                            dcn_layers=dcn_layers[0], dcn_interval=dcn_interval)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, 
                            dcn_layers=dcn_layers[1], dcn_interval=dcn_interval)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, 
                            dcn_layers=dcn_layers[2], dcn_interval=dcn_interval)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, 
                            dcn_layers=dcn_layers[3], dcn_interval=dcn_interval)
    ...
    def load_pretrained_params(self):
        state_dict = model_zoo.load_url(self.model_url)
